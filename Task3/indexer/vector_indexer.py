...
def create_vector_index(csv_folder: str, output_path: str = "./vector_index"):
    """
    Полный пайплайн создания векторного индекса
    
    Args:
        csv_folder: папка с CSV файлами
        output_path: путь для сохранения индекса
    """
    # Инициализация индексатора
    indexer = CSVVectorIndexer()
    
    # 1. Загрузка CSV файлов
    print("Шаг 1: Загрузка CSV файлов...")
    documents = indexer.load_csv_files(csv_folder)
    ...
    chunks = indexer.chunk_documents(documents, chunk_size=500, chunk_overlap=50)
    ...
    print("\nШаг 4: Построение индекса FAISS...")
    ...
    results = indexer.search("пример запроса для теста", k=3)
    for result in results:
        print(f"\nРезультат {result['rank']} (сходство: {result['score']:.3f}):")
        print(f"Источник: {result['source_file']}, строка: {result['original_row']}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Создание векторного индекса из CSV файлов')
    ...
    parser.add_argument('--output_path', type=str, default='./vector_index',
                       help='Путь для сохранения индекса (по умолчанию: ./vector_index)')
    
    args = parser.parse_args()
    
    # Создаем индекс
    indexer = create_vector_index(args.csv_folder, args.output_path)
    
    print(f"\n✅ Индекс успешно создан и сохранен в {args.output_path}")
    print("\nДля использования в боте:")
    print(f"bot = KnowledgeBaseBot('{args.output_path}')")
    print("answer = bot.get_detailed_answer('Ваш вопрос')")